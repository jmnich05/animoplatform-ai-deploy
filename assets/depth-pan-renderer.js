...
async function textureFromImage(image) {
  const texture = await new THREE.TextureLoader().loadAsync(image.currentSrc || image.src);
...
/**
 * Adds a Tiefling-depth-driven Three.js layer to the existing image explorer.
...
export async function mountDepthPanRenderer({
  ...
  try {
    ...
    colorTexture = await textureFromImage(image);
